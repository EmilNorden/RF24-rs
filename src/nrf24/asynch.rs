...
use crate::nrf24::{Async, Nrf24l01, NrfResult, Sync};
...
impl<SPI, CE> Nrf24l01<SPI, CE, Async>
where
    SPI: SpiDevice,
    CE: OutputPin,
{
    ...
    /// match Nrf24l01::new_async(spi, ce, &mut delay, config).await {
    ///     Ok(nrf) => {
    ...
    pub async fn new_async<D: DelayNs>(
        spi: SPI,
        ce: CE,
        delay: &mut D,
        config: NrfConfig,
        ...
        chip.set_ack_payloads_enabled(config.ack_payloads_enabled).await?;
        // flush buffers
        chip.flush_rx().await?;
        chip.flush_tx().await?;
    ...
    /// # Examples
    /// ```rust
    /// if !chip.is_connected().await? {
    ///     // Handle disconnection
    /// }
    /// ```
    pub async fn is_connected(&mut self) -> NrfResult<bool, SPI, CE> {
        self.read_register(Register::SETUP_AW).await
    ...
    /// # Examples
    /// ```rust
    /// chip.open_reading_pipe(DataPipe::DP0, b"Node1").await?;
    /// ```
    ///
    ...
    pub async fn open_reading_pipe<T: Into<DataPipe>>(
        ...
        match pipe {
            DataPipe::DP0 | DataPipe::DP1 => self.write_register(rx_address_reg, addr).await?,
            _ => self.write_register(rx_address_reg, addr[0]).await?,
        }

        // Enable corresponding RX Addr
        let old_reg = self.read_register(Register::EN_RXADDR).await?; // Read old value
        self.write_register(Register::EN_RXADDR, old_reg | (1 << pipe.pipe())).await?; // Update
    ...
    /// chip.open_writing_pipe(b"Node1").await?;
    /// ```
    /// # Warnings
    ...
    pub async fn open_writing_pipe(&mut self, mut addr: &[u8]) -> NrfResult<(), SPI, CE> {
        ...
        self.write_register(Register::RX_ADDR_P0, addr).await?;
        // Open writing pipe
        self.write_register(Register::TX_ADDR, addr).await?;
    ...
    /// while chip.data_available().await? {
    ///     // ... read the payload
    ...
    pub async fn data_available(&mut self) -> NrfResult<bool, SPI, CE> {
        Ok(self.data_available_on_pipe().await?.is_some())
    }
    ...
    /// while let Some(pipe) = chip.data_available_on_pipe().await? {
    ...
    /// }
    /// ```
    pub async fn data_available_on_pipe(&mut self) -> NrfResult<Option<DataPipe>, SPI, CE> {
        Ok(self.status().await?.data_pipe_available())
    }
    ...
    /// let chip = Nrf24l01::new_async(spi, ce, &mut delay, config).await.unwrap();
    ...
    ///     while let Ok(true) = chip.data_available().await {
    ...
    pub async fn read(&mut self, buf: &mut [u8]) -> NrfResult<usize, SPI, CE> {
        let len = match self.payload_size {
            ...
            PayloadSize::Dynamic => {
                let payload_len = self.read_rx_payload_width().await?;
    ...
    /// // Set the payload size to 4 bytes, the size of an f32
    /// let config = NrfConfig::default().payload_size(PayloadSize::Static(4));
    /// let chip = Nrf24l01::new_async(spi, ce, &mut delay, config).unwrap();
    ...
    ///
    ///     match chip.write(&mut delay, &buffer).await {
    ...
    pub async fn write<D: DelayNs>(&mut self, delay: &mut D, buf: &[u8]) -> NrfResult<(), SPI, CE> {
    ...
    /// nrf24l01.set_retries((5, 15)).await?;
    /// ```
    pub async fn set_retries<T: Into<AutoRetransmission>>(
        &mut self,
        auto_retry: T,
    ) -> NrfResult<(), SPI, CE> {
    ...
    /// ```rust
    /// // Initialize the chip
    /// let mut chip = Nrf24l01::new_async(spi_struct, ce_pin, delay, NrfConfig::default()).await?;
    ///
    /// let retries_config = chip.retries().await?;
    /// // Default values for the chip
    /// assert_eq!(retries_config.delay(), 1586);
    /// assert_eq!(retries_config.count(), 15);
    /// ```
    pub async fn retries(&mut self) -> NrfResult<AutoRetransmission, SPI, CE> {
        self.read_register(Register::SETUP_RETR).await
            .map(AutoRetransmission::from_register)
    }

    /// Returns the number of retransmissions in the last transaction.
    pub async fn retries_in_last_transmission(&mut self) -> NrfResult<u8, SPI, CE> {
        let value =  self.read_register(Register::OBSERVE_TX).await?;
        Ok(value & 0b111)
    }
    ...
    /// # Examples
    /// ```rust
    /// nrf24l01.set_channel(74).await?;
    /// ```
    pub async fn set_channel(&mut self, channel: u8) -> NrfResult<(), SPI, CE> {
    ...
    /// # Examples
    /// ```rust
    /// nrf24l01.set_ack_payloads_enabled(true).await?;
    pub async fn set_ack_payloads_enabled(&mut self, enabled: bool) -> NrfResult<(), SPI, CE> {
        let feature = self.read_register(Register::FEATURE).await?;
        if enabled {
            self.write_register(Register::FEATURE, feature | (1 << 1)).await?
        } else {
            self.write_register(Register::FEATURE, feature & !(1 << 1)).await?
        }
    ...
    /// assert_eq!(chip.channel().await?, 76);
    /// ```
    ...
    /// # Examples
    /// ```rust
    /// nrf24l01.set_address_width(5).await?;
    /// ```
    pub async fn set_address_width<T>(&mut self, width: T) -> NrfResult<(), SPI, CE>
    where
        T: Into<AddressWidth>,
    {
        let width = width.into();
        self.write_register(Register::SETUP_AW, width.value()).await
    }
    ...
    /// let mut chip = Nrf24l01::new_async(spi_struct, ce_pin, delay, NrfConfig::default()).await?;
    /// // Default is 2 Mb/s
    /// assert_eq!(chip.data_rate().await?, DataRate::R2Mbps);
    /// ```
    pub async fn data_rate(&mut self) -> NrfResult<DataRate, SPI, CE> {
        self.read_register(Register::RF_SETUP).await.map(DataRate::from)
    }
    ...
    /// assert_eq!(chip.power_amp_level().await?, PALevel::Min);
    /// ```
    pub async fn power_amp_level(&mut self) -> NrfResult<PALevel, SPI, CE> {
        self.read_register(Register::RF_SETUP).await.map(PALevel::from)
    }
    ...
    /// # Example
    /// ```rust
    /// nrf24l01.write_ack_payload(DataPipe::DP0, payload_buffer).await??;
    pub async fn write_ack_payload(&mut self, pipe: DataPipe, payload: &[u8]) -> NrfResult<(), SPI, CE> {
        self.spi
            .transaction(&mut [
    ...
    /// # Examples
    /// ```rust
    /// chip.enable_crc(EncodingScheme::R2Bytes).await?;
    /// ```
    pub async fn enable_crc(&mut self, scheme: EncodingScheme) -> NrfResult<(), SPI, CE> {
    ...
    /// # Examples
    /// ```rust
    /// match chip.crc_encoding_scheme().await? {
    ...
    /// };
    /// ```
    pub async fn crc_encoding_scheme(&mut self) -> NrfResult<EncodingScheme, SPI, CE> {
        self.read_register(Register::CONFIG).await.map(From::from)
    }
    ...
    /// chip.set_payload_size(PayloadSize::Dynamic).await?;
    /// chip.set_payload_size(0).await?;
    /// // Following methods set a static payload size.
    /// chip.set_payload_size(12).await?; // Messages will be 12 bytes
    /// chip.set_payload_size(PayloadSize::Static(12)).await?; // Same as previous
    /// chip.set_payload_size(49).await?; // Messages will be `MAX_PAYLOAD_SIZE`
    /// ```
    pub async fn set_payload_size<T: Into<PayloadSize>>(
        &mut self,
        payload_size: T,
    ) -> NrfResult<(), SPI, CE> {
        ...
        match payload_size {
            PayloadSize::Static(payload_size) => {
                ...
                self.write_register(Register::RX_PW_P0, payload_size).await?;
                self.write_register(Register::RX_PW_P1, payload_size).await?;
                self.write_register(Register::RX_PW_P2, payload_size).await?;
                self.write_register(Register::RX_PW_P3, payload_size).await?;
                self.write_register(Register::RX_PW_P4, payload_size).await?;
                self.write_register(Register::RX_PW_P5, payload_size).await?;
            }
            PayloadSize::Dynamic => {
                let feature = self.read_register(Register::FEATURE).await?;
                self.write_register(Register::FEATURE, feature | (1 << 2)).await?;
                self.write_register(Register::DYNPD, 0b0001_1111).await?; // enable on all pipes
            }
        }
    ...
    /// ```rust
    /// // Go to sleep
    /// chip.power_down(&mut delay).await?;
    /// // Zzz
    /// // ...
    /// chip.power_up(&mut delay).await?; // power back up
    /// ```
    ...
    pub async fn reset_status(&mut self) -> NrfResult<(), SPI, CE> {
        self.write_register(Register::STATUS, Self::STATUS_RESET).await
    }
    ...
    /// chip.mask_interrupts(interrupts).await?;
    /// ```
    /// Disable all interrupts.
    /// ```rust
    /// let interrupts = Interrupts::all();
    /// chip.mask_interrupt(interrupts).await;
    /// ```
    pub async fn mask_interrupts(&mut self, irq: Interrupts) -> NrfResult<(), SPI, CE> {
        // Clear interrupt flags
    ...
    pub async fn interrupt_src(&mut self) -> NrfResult<Interrupts, SPI, CE> {
    ...
    pub async fn read_config(&mut self) -> NrfResult<NrfConfig, SPI, CE> {
        let addr_width = AddressWidth::from_register(self.read_register(Register::SETUP_AW).await?);
        let config = NrfConfig::default()
            .payload_size(self.payload_size)
            .channel(self.channel().await?)
            .addr_width(addr_width)
            .data_rate(self.data_rate().await?)
            .pa_level(self.power_amp_level().await?)
            .crc_encoding_scheme(self.crc_encoding_scheme().await?)
            .auto_retry(self.retries().await?);
        Ok(config)
    }

    pub async fn rx_fifo_empty(&mut self) -> NrfResult<bool, SPI, CE> {
        Ok((self.read_register(Register::FIFO_STATUS).await? & 1) == 1)
    }
    ...
    /// as for arrays.
    pub(crate) async fn write_register<T: IntoBuf<u8>>(
        &mut self,
        register: Register,
        buf: T,
    ) -> NrfResult<(), SPI, CE> {
    ...
    pub(crate) async fn read_register(&mut self, register: Register) -> NrfResult<u8, SPI, CE> {
        let mut buf = [0_u8];
        self.spi
            .transaction(&mut [
    ...
    pub(crate) async fn setup_rf(&mut self, data_rate: DataRate, level: PALevel) -> NrfResult<(), SPI, CE> {
...
where
    ...
    CE: OutputPin,
{
    pub(crate) fn set_ce_high(&mut self) -> NrfResult<(), SPI, CE> {
        self.ce.set_high().map_err(TransceiverError::Ce)
    }
    pub(crate) fn set_ce_low(&mut self) -> NrfResult<(), SPI, CE> {
